...
(function ($) {
    ...
    /*------------------
        Magnific Popup - Ativado somente na imagem grande
    --------------------*/
    $('.car__details__pic__large').on('click', function() {

     // Array para armazenar as URLs de todas as fotos
     var photos = [];

     // Adiciona a imagem atualmente exibida na div grande
     var currentLargeImgSrc = $('.car-big-img').attr('src');
     if (currentLargeImgSrc) {
         photos.push({
             src: currentLargeImgSrc,
             type: 'image'
         });
     }

     // Adiciona as URLs dos thumbnails ao array de fotos
     $('.car-thumbs .ct img').each(function() {
         var thumbSrc = $(this).attr('src');
         // Evita adicionar a mesma foto duas vezes (se a principal já estiver no array)
         if (thumbSrc && thumbSrc !== currentLargeImgSrc) {
             photos.push({
                 src: thumbSrc,
                 type: 'image'
             });
     ...
     // Abre o popup com o array de fotos
     $.magnificPopup.open({
         items: photos,
         gallery: {
             enabled: true
         ...
         type: 'image'
     }, 0); // O '0' indica para abrir a partir do primeiro item da galeria (a imagem grande)
    });
